...
class Shanty_Mongo_DocumentSet extends Shanty_Mongo_Document
{
	...
	public function getProperty($index = null)
	{
		...
		if (!$new && array_key_exists($index, $this->_data)) {
			...
			// If this breaks AsReference Data Saving, I'll be mad - corey
			if (MongoDBRef::isRef($data)) {
				$data['$ref'] = $collection = Shanty_Mongo::getCollectionRenamed($data['$ref']);
				$data = MongoDBRef::get(static::getMongoDB(), $data);
				if(!$data) return null;
				$config = array ();
				$config['collection'] = $collection;
				$config['requirementModifiers'] = $this->getRequirements(static::DYNAMIC_INDEX.'.');
				$config['parentIsArray'] = true;
				$config['hasId'] = $this->hasRequirement(static::DYNAMIC_INDEX, 'hasId');
		...
		if ($override = $this->getPropertyClass($index, $data)) {
			$className = $override;
		}
	...
	public function setProperty($index, $document)
	{
		...
		if (!($document instanceof Shanty_Mongo_Document)) {
			require_once 'Shanty/Mongo/Exception.php';
			throw new Shanty_Mongo_Exception("Document must be instance of Shanty_Mongo_Document");
		}

		// Make sure we are not keeping a copy of the old document in reference memory
		if (!$new && isset($this->_data[$index]) && is_object($this->_data[$index])) {
			$this->_references->detach($this->_data[$index]);				
		}

		// Throw exception if value is not valid
		$validators = $this->getValidators(static::DYNAMIC_INDEX);
		if (!$validators->isValid($document)) {
			require_once 'Shanty/Mongo/Exception.php';
			throw new Shanty_Mongo_Exception(implode($validators->getMessages(), "\n"));
		}

		if ($this->hasRequirement(static::DYNAMIC_INDEX, 'AsReference')) {
			$document = $document->createReference();
		}
...
//		$value = $this->getFilters(self::DYNAMIC_INDEX)->filter($document);
		...
		if ($new) {
			...
			if (empty($keys)) $index=0;
			else $index = max($keys)+1;
			$this->_data[$index] = $document;
		}
	...
	public function export()
	{
		...
		if(empty($exportData)) return array();
	 ...
	 * @param String $property is for debugging loops
	 */
	public function purgeOperations($includingChildren = false, $property = false)
	{
		if ($this->hasRequirement(self::DYNAMIC_INDEX, 'AsReference')) $includingChildren = false;

		return parent::purgeOperations($includingChildren, $property);
	}
