...
class Shanty_Mongo_Connection_Group
{
	...
	public function getWriteConnection()
	{
		...
		if ($write && !$write->connected) $write->connect();
	...
	public function getReadConnection()
	{
		if (count($this->_slaves) === 0) {
			...
			if ($read && !$read->connected) $read->connect();
		}
	...
	public function formatHostString(array $hostOptions = array())
	{
		...
		// Set username
		if (isset($hostOptions['username']) && !empty($hostOptions['username'])) {
			$hostString .= $hostOptions['username'];
			
			// Set password
			if (isset($hostOptions['password']) && !empty($hostOptions['password'])) {
