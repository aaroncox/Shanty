...
/**
	...
	public function getProperty($property)
	{
		...
		if (MongoDBRef::isRef($data)) {
			...
			// $this->_data['_parent']['$ref'] = 'something';
			// $collection = $data['$ref'];
		...
		if ($override = $this->getPropertyClass($property, $data))
		  $className = $override;
	...
	public function setProperty($property, $value)
	{
		...
		if ($this->hasRequirement($property, "Int")) {
			$value = (int)$value;
		}
		
		if ($this->hasRequirement($property, "Float")) {
			$value = (float)$value;
		}
		
		if ($this->hasRequirement($property, "Boolean")) {
			$value = !!$value;
		}
    ...
    $this->checkRequirements();
		...
		$result = $mongoCollection->update($this->getCriteria(), $operations, array('upsert' => true, 'safe'=>true));
	...
	public function purgeOperations($includingChildren = false, $property = false)
	{
		if ($includingChildren) {
			foreach ($this as $property => $document) {
				...
				if (!($this->isReference($document) || $this->hasRequirement($property, 'AsReference'))) {
					$document->purgeOperations(true, $property);
				}
			}
		}
